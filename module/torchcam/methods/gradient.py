...
class _GradCAM(_CAM):
    ...
    def _backprop(self, scores: Tensor, class_idx: Union[int, List[int]], retain_graph: bool = False) -> None:
        ...
        if isinstance(class_idx, int):
            ...
            # if len(scores) > 1:
            #     loss = scores[0][:, class_idx].sum()
            # else:
            #     loss = scores[:, class_idx].sum()
        else:
            loss = scores.gather(1, torch.tensor(class_idx, device=scores.device).view(-1, 1)).sum()
            # if len(scores) > 1:
            #     loss = scores[0].gather(1, torch.tensor(class_idx, device=scores[0].device).view(-1, 1)).sum()
            # else:
            #     loss = scores.gather(1, torch.tensor(class_idx, device=scores[0].device).view(-1, 1)).sum()
...
class SmoothGradCAMpp(_GradCAM):
    ...
    >>> from module.torchcam.methods.gradient import SmoothGradCAMpp
    ...
    def _get_weights(
        ...
        for _idx in range(self.num_samples):
            ...
            # Forward & Backward
            if text_included:
                out,_ = self.model(noisy_input, text=text_tensor, text_included=True)
            else:
